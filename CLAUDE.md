...
- Veröffentlichen: Inhalt von `publish\` auf den `gh-pages`-Branch pushen (eigener, von `main` unabhängiger Branch — sonst würde jede Version mehrere hundert MB Binärdaten dauerhaft in die `main`-Historie schreiben). GitHub Pages ist in den Repo-Einstellungen auf „Deploy from branch: gh-pages / (root)“ konfiguriert.
