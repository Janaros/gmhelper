...
Lizenzschlüssel wird **nicht** eingecheckt: `App.xaml.cs` liest optional `<Projektordner>\syncfusion-license.local.txt` (per `.gitignore` ausgeschlossen) und registriert ihn via `Syncfusion.Licensing.SyncfusionLicenseProvider.RegisterLicense(...)`. Ohne diese Datei läuft die App im unlizenzierten Trial-Modus (Hinweisdialog/Wasserzeichen), stürzt aber nicht ab. Installierte Kopien anderer Nutzer haben diese Datei nie (sie ist an den Account des Entwicklers gebunden und darf nicht mitverteilt werden) und laufen deshalb bewusst dauerhaft im Trial-Modus.

## Installer / Distribution

GMHelper wird per **ClickOnce** verteilt, gehostet auf **GitHub Pages** unter `https://janaros.github.io/gmhelper/`. Repo: `https://github.com/Janaros/gmhelper` (öffentlich, notwendig für kostenloses GitHub-Pages-Hosting).

- Das .NET-SDK (`dotnet publish`) unterstützt die ClickOnce-Manifest-Tasks nicht (`MSB4803: GenerateTrustInfo`) — dafür wird das **Full-Framework-MSBuild aus einer Visual-Studio-Installation** benötigt (`vswhere.exe` → `MSBuild\Current\Bin\amd64\MSBuild.exe`).
- Publish-Profil: `src/GMHelper.App/Properties/PublishProfiles/ClickOnce.pubxml` (self-contained `win-x64`, unsigniert — kein Code-Signing-Zertifikat vorhanden, daher zeigt Windows beim ersten Start eine "Unbekannter Herausgeber"-Warnung).
- Build/Deploy-Schritt: `scripts/Publish-ClickOnce.ps1` — liest `<Version>` aus `GMHelper.App.csproj` (dieselbe Versionsnummer, die auch im About-Dialog erscheint), baut `Configuration=Release` und veröffentlicht nach `<Projektordner>\publish\` (gitignored, lokal generiert). Skript-CLI-Properties überschreiben bewusst die Werte aus der `.pubxml`, weil sich `Configuration=Release` aus der `.pubxml` allein nicht auf referenzierte Projekte (Core/Data/Services) durchsetzt — ohne die explizite Kommandozeilen-Property wurden dort still Debug-DLLs eingepackt.
- Veröffentlichen: `scripts/Deploy-GhPages.ps1` (ruft `Publish-ClickOnce.ps1` selbst auf) pusht den Inhalt von `publish\` auf den `gh-pages`-Branch (eigener, von `main` unabhängiger Branch — sonst würde jede Version mehrere hundert MB Binärdaten dauerhaft in die `main`-Historie schreiben). GitHub Pages ist in den Repo-Einstellungen auf „Deploy from branch: gh-pages / (root)“ konfiguriert.
- **Wichtige Falle**: Die `.manifest`/`.application`-Dateien sind Text-XML mit einem eingebetteten SHA-256-Hash ihrer eigenen exakten Bytes. Wenn Git beim Commit die Zeilenenden normalisiert (Windows-Standard `core.autocrlf=true`), ändert sich der Hash nicht mit — jeder Client meldet dann beim Start "Die Gültigkeit der Anwendung konnte nicht überprüft werden", obwohl lokal alles funktioniert (das ist uns beim ersten Deploy genau so passiert). `Deploy-GhPages.ps1` deployt deshalb über einen frischen Klon mit `core.autocrlf false` und einer `.gitattributes` (`* -text -diff`) auf dem `gh-pages`-Branch — beides niemals umgehen.
- **Jede neu veröffentlichte Version braucht eine neue `<Version>`** in `GMHelper.App.csproj` (ClickOnce erkennt Updates am 4-teiligen `ApplicationVersion`, das Skript hängt automatisch `.0` an `<Version>` an) — deckt sich mit der bestehenden Vorgabe, die Versionsnummer bei jeder ausgelieferten Änderung zu erhöhen.
- `App.xaml.cs`s Datenordner-Fallback (`%LocalAppData%\GMHelper`, siehe Abschnitt „Datenablage“ oben) ist die Voraussetzung dafür, dass installierte Kopien bei jedem ClickOnce-Update (neuer, versionsspezifischer Cache-Ordner) ihre Daten behalten.
